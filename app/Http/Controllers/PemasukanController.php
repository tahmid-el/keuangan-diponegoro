...
class PemasukanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->keterangan as $index => $ket) {
            ...
            if ($request->hasFile("bukti.$index")) {
                $data['bukti'] = $request->file("bukti.$index")->store('bukti/pemasukan', 'public');
            }
    ...
    private function resolveKategoriId(Request $request): int
    {
        ...
        return Kategori::firstOrCreate(
            ['nama' => trim($request->kategori_baru), 'tipe' => 'pemasukan'],
            ['kelompok_isak35' => 'Pendapatan', 'status_pembatasan_dana' => 'Tanpa Pembatasan']
        )->id;
    }
}
