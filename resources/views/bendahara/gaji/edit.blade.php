...
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="d-flex align-items-center mb-3">
            ...
            <div>
                <h4 class="fw-bold text-dark mb-0">Edit Data Gaji</h4>
                <p class="text-muted small mb-0">Perbarui data gaji guru atau karyawan</p>
            </div>
        </div>
        ...
        <div class="glass-card p-4 slide-up">
            <form action="{{ route('bendahara.gaji.update', $gaji->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" name="nama"
                               placeholder="Contoh: Ahmad Dahlan" value="{{ old('nama', $gaji->nama) }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label-sm">Jumlah Jam</label>
                        <input type="number" class="form-control" name="jumlah_jam"
                               placeholder="Contoh: 40" value="{{ old('jumlah_jam', $gaji->jumlah_jam) }}" min="0" required>
                    </div>
                </div>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control nominal-input" name="bisyaroh"
                               placeholder="0" value="{{ old('bisyaroh', number_format((float)$gaji->bisyaroh, 0, ',', '.')) }}" required>
                    </div>
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control nominal-input" name="tunjangan_kamad_wk"
                               placeholder="0" value="{{ old('tunjangan_kamad_wk', number_format((float)$gaji->tunjangan_kamad_wk, 0, ',', '.')) }}" required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label-sm">Tunjangan Piket</label>
                        <input type="text" class="form-control nominal-input" name="tunjangan_piket"
                               placeholder="0" value="{{ old('tunjangan_piket', number_format((float)$gaji->tunjangan_piket, 0, ',', '.')) }}" required>
                    </div>
                </div>

                <div class="d-flex justify-content-end align-items-center mt-2 mb-3 px-1">
                    <span class="text-muted small me-2">Total Gaji:</span>
                    <span id="total-nominal-display" class="fw-bold fs-5 text-success">Rp {{ number_format((float)$gaji->jumlah, 0, ',', '.') }}</span>
                </div>
